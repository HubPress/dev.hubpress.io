...
== What Is HubPress?
image::/docs/images/what-is-hubpress.gif[]
...
Read the next two sections *carefully* _*before*_ choosing which branch and `config.json` file to edit. 
...
+
* `username`, which is your GitHub user name,
* `repositoryName`, which is the new name of the repository fork. For example, `<username>.github.io`.
...
If your issue is not covered by the Knowledgebase, join hubpressio.slack.com[HubPress Slack] and report the issue in #general.
