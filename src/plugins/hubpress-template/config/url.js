  ...
  if (_.isObject(context) && context.relativeUrl) {
    ...
    if (context === 'post' && data.post) {
      ...
      urlPath = urlJoin(
        getBaseUrl(),
        '/',
        ghostConfig.routeKeywords.author,
        data.author.slug,
        '/',
      )
      ...
      imagePathRe = new RegExp(
        '^' + ghostConfig.paths.subdir + '/' + ghostConfig.paths.imagesRelPath,
      )
      ...
      if (absolute) {
        ...
        urlPath = urlPath.replace(
          new RegExp('^' + ghostConfig.paths.subdir),
          '',
        )
...
function apiUrl() {
  ...
  if (ghostConfig.forceAdminSSL) {
    url = (ghostConfig.urlSSL || ghostConfig.url)
      .replace(/^.*?:\/\//g, 'https://')
