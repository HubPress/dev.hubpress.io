...
get = function get(context, options) {
  ...
  apiMethod = isBrowse(context, apiOptions)
    ? api[context].browse
    : api[context].read
  // Parse the options we're going to pass to the API
  apiOptions = parseOptions(this, apiOptions)

  return apiMethod(apiOptions)
...
module.exports = function getWithLabs(context, options) {
  var self = this,
    errorMessages = [
      ...
      i18n.t('warnings.helpers.get.seeLink', {
        url: 'http://support.ghost.org/public-api-beta',
      }),
    ]
  ...
  if (labs.isSet('publicAPI') === true) {
    ...
    return Promise.resolve(function noGetHelper() {
      return (
        '<script>console.error("' + errorMessages.join(' ') + '");</script>'
      )
    })
  }
}
