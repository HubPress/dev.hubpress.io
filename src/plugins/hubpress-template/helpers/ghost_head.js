...
function ghost_head(options) {
  ...
  var metaData = getMetaData(this, options.data.root),
    ...
    context = this.context
      ? Array.isArray(this.context) ? this.context[0] : this.context
      : null,
  ...
  if (context) {
    ...
    if (metaData.nextUrl) {
      head.push(
        '<link rel="next" href="' + escapeExpression(metaData.nextUrl) + '" />',
      )
    }
  ...
  head.push(
    utils.stylesheetTemplate({
      source:
        '//cdnjs.cloudflare.com/ajax/libs/highlight.js/9.10.0/styles/atom-one-dark.min.css',
    }),
  )
  head.push(
    `
