...
<div class="posts-container">
    ...
    <div class="posts ui container centered">
        ...
        <div class="ui cards centered aligned">
            <div class="ui card" v-bind:class="getPostStatusColor(post)" v-for="post in posts" :key="post.id">
                <div class="content">
                    <div class="header">{{post.title}}</div>
                    <div class="meta">
                        <span class="right floated time">{{ publishedAt(post) }}</span>
                        <span class="status">{{type(post)}} - {{ status(post) }}</span>
                    </div>
                </div>
                ...
                <div class="extra content">
                    <a class="ui tiny label" v-for="tag in post.tags" :key="tag">
...
export default {
  ...
  methods: {
    ...
    type: function(post) {
      return post.type.replace(/\b\w/g, l => l.toUpperCase())
    },
    publishedAt: function(post) {
      return (!!post.published && post.published_at && moment(post.published_at).fromNow()) || ''
    },
