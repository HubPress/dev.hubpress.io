    ...
    const hubpress = {
      ...
      mutations: {
        ...
        [APPLICATION_INITIALIZE_PLUGINS](state, nextRootState) {
          console.log(
            'hubpress-' + APPLICATION_INITIALIZE_PLUGINS,
            nextRootState,
          )
...
`
      ...
      actions: {
        ...
        [POSTS_SYNCHRONIZE]({ dispatch, commit, rootState, state }) {
          const opts = {
            rootState: _.cloneDeep(rootState),
            currentState: _.cloneDeep(state),
          }
          return dispatch('application:loading')
            ...
            .then(updatedOpts =>
              commit(POSTS_SYNCHRONIZE, updatedOpts.nextState),
            )
        ...
        [POST_DELETE]({ dispatch, commit, rootState, state }, postId) {
        ...
        [POST_REMOTE_SAVE]({ dispatch, commit, rootState, state }, postId) {
        ...
        [POST_PUBLISH]({ dispatch, commit, rootState, state }, postId) {
        ...
        [POST_UNPUBLISH]({ dispatch, commit, rootState, state }, postId) {
          const opts = {
            rootState: _.cloneDeep(rootState),
            currentState: _.cloneDeep(state),
          }
        ...
        [POSTS_GET]({ dispatch, commit, rootState, state }) {
        ...
        [POST_CHANGE_CONTENT](
          ...
          return logic.renderAndSavePost(opts).then(updatedOpts => {
  ...
  context.on('application:prepare-config', opts => {
    ...
    opts.nextState.config.site.title = opts.payload.formData.get(
      'hubpress-title',
    )
    opts.nextState.config.site.description = opts.payload.formData.get(
      'hubpress-description',
    )
    opts.nextState.config.site.logo = opts.payload.formData.get('hubpress-logo')
    opts.nextState.config.site.cover = opts.payload.formData.get(
      'hubpress-cover-image',
    )
    opts.nextState.config.site.delay = opts.payload.formData.get(
    ...
    opts.nextState.config.theme.name = opts.payload.formData.get(
      'hubpress-theme',
    )

    // Config Social network
    opts.nextState.config.socialnetwork =
      opts.nextState.config.socialnetwork || {}
    opts.nextState.config.socialnetwork.email = opts.payload.formData.get(
      'social-email',
    )
    opts.nextState.config.socialnetwork.github = opts.payload.formData.get(
      'social-github',
    )
    opts.nextState.config.socialnetwork.twitter = opts.payload.formData.get(
      'social-twitter',
    )
    opts.nextState.config.socialnetwork.facebook = opts.payload.formData.get(
      'social-facebook',
    )
    opts.nextState.config.socialnetwork.googleplus = opts.payload.formData.get(
      'social-googleplus',
    )
    opts.nextState.config.socialnetwork.instagram = opts.payload.formData.get(
      'social-instagram',
    )
    opts.nextState.config.socialnetwork.pinterest = opts.payload.formData.get(
      'social-pinterest',
    )
    opts.nextState.config.socialnetwork.flickr = opts.payload.formData.get(
      'social-flickr',
    )
    opts.nextState.config.socialnetwork.linkedin = opts.payload.formData.get(
      'social-linkedin',
    )
    ...
    opts.nextState.config.navigation = []
      for (const key of opts.payload.formData.keys()) {
        const navigationKeyLabel = 'navigation-label-'
        if (key.startsWith(navigationKeyLabel)) {
          const label = opts.payload.formData.get(
            key
          )
          const indexFormdata = key.split(navigationKeyLabel)[1]
          const url = opts.payload.formData.get(`navigation-url-${indexFormdata}`)
          opts.nextState.config.navigation.push({
            label,
            url
          })
        }
  ...
  context.on('application:save-config-done', opts => {
    ...
    return logic.refreshAfterSavedConfig(localOpts).then(updatedOpts => {
      // Then we set our localOpts.nextState to the hubpress state
      opts.nextState.hubpress = updatedOpts.nextState
      return opts
    })
